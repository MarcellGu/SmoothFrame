...
`squircle-frame` 是一个独立、零依赖的 Rust crate，用来生成 Sketch-like smooth corner / squircle frame 的 cubic Bezier 路径。
...
仓库内置了一个 binary 示例，可以直接生成 SVG path：
...
cargo run --bin demo -- --output squircle.svg --border 4
```

默认生成 `1000 x 1000`、角半径为 `250` 的 smooth corner SVG。生成 SVG 文件时会按边框宽度向内收缩路径，并将路径平移半个边框宽度，避免 stroke 被裁切。
...
也就是 4 条边和 4 个角，每角最多 3 段 cubic，总计最多 12 段 cubic。与 `sketchtool` 一样，矩形便捷 API 会省略零长度边或首尾零长度 cubic；当半径进入近圆或近 capsule 区间时，也会退化为 SketchTool 的普通圆角、圆或 capsule-like 结构。
...
```rust
use squircle_frame::{Point, SmoothCorner, Vector};
...
.to_cubic_segments()?;
# Ok::<_, squircle_frame::SmoothError>(())
```
...
```rust
use squircle_frame::{Point, SmoothFrame};
...
v1 支持闭合凸 polygon。凹角目前返回 `SmoothError::ConcaveFrame`，自相交路径返回 `SmoothError::SelfIntersectingFrame`，API 设计保留后续支持 concave corner 的空间。
...
矩形便捷 API 在近圆或近 capsule 区间继续以 `sketchtool` 为准：当 SketchTool 退化为普通圆角、圆或 capsule-like 结构时，`SmoothRect` 也做同样退化。
...
- 集成测试会在本机存在 `sketchtool` 时，通过 `sketchtool run-script` 在内存中逐个创建 smooth rect，导出 SVG Buffer，立刻删除该 shape，再继续下一个用例。
...
```bash
SQUIRCLE_FRAME_SKETCHTOOL=/Applications/Sketch.app/Contents/MacOS/sketchtool cargo test
SQUIRCLE_FRAME_REQUIRE_SKETCHTOOL=1 cargo test
```
...
```rust
let path = squircle_frame::SmoothRect::new(1000.0, 500.0)
    .with_radius(250.0)
    .with_smoothing(0.6)
    .to_path();
